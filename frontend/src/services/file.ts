...
export class FileService {
  ...
  /** 秒传检查：通过文件哈希查询是否已存在相同文件 */
  static async dedupCheck(request: {
    file_hash: string
    file_size: number
    file_name: string
    expire_value: number
    expire_style: string
    text?: string
  }): Promise<ApiResponse<{ existed: boolean; code?: string; name?: string; size?: number }>> {
    return api.post('/share/dedup/check', request)
  }
