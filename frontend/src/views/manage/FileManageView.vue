        ...
        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          <label class="space-y-1">
            <span class="flex items-center text-xs font-medium" :class="[mutedTextClass]">
              ...
              {{ t('fileManage.dedupLabel') }}
            </span>
            <select
              :value="params.dedup"
              class="w-full rounded-lg border px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-zinc-500"
              :class="[fieldClass]"
              @change="handleDedupFilterChange"
            >
              <option v-for="option in dedupFilterOptions" :key="option.value" :value="option.value">
                {{ option.label }}
              </option>
            </select>
          </label>
        ...
        <section class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs" :class="[mutedTextClass]">
              下载 {{ selectedFileDetail.downloadCount }} 次 · 共 {{ selectedFileDetail.auditLogs.length }} 条记录
            </span>
          </div>
          <div
            class="max-h-64 space-y-2 overflow-y-auto rounded-lg border px-4 py-3"
            :class="[isDarkMode ? 'border-white/10 bg-white/[0.04]' : 'border-zinc-200/70 bg-white/60']"
          >
            <div v-if="selectedFileDetail.auditLogs.length === 0" class="py-4 text-center text-xs" :class="[mutedTextClass]">
              暂无审计记录（历史上传的文件可能缺少关联信息）
            </div>
            <div
              v-for="log in selectedFileDetail.auditLogs"
              :key="log.id"
              class="flex items-start gap-3 rounded-md border px-3 py-2"
              :class="[isDarkMode ? 'border-gray-700/60 bg-gray-800/40' : 'border-gray-100 bg-gray-50/80']"
            >
              <span
                class="mt-0.5 inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-[10px] font-medium"
                :class="log.event_type === 'upload' ? 'bg-blue-100 text-blue-700' : log.event_type === 'download' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'"
              >
                {{ log.event_type === 'upload' ? '上传' : log.event_type === 'download' ? '下载' : log.event_type }}
              </span>
              <div class="min-w-0 flex-1">
                <p class="text-xs" :class="[primaryTextClass]">
                  {{ log.ip }}
                  <span v-if="log.ip_location" class="ml-1" :class="[mutedTextClass]">({{ log.ip_location }})</span>
                </p>
                <p v-if="log.detail && log.detail.note" class="mt-0.5 text-[11px] font-medium text-orange-600">
                  {{ log.detail.note }}
                </p>
                <p class="mt-0.5 text-[11px]" :class="[mutedTextClass]">
                  {{ log.device_type || '' }} {{ log.browser || '' }} · {{ log.os || '' }}
                </p>
              </div>
              <span class="shrink-0 text-[11px]" :class="[mutedTextClass]">
...
const dedupFilterOptions = computed<{ value: AdminFileDedupFilter; label: string }[]>(() => [
  { value: 'all', label: t('fileManage.all') },
  { value: 'dedup', label: t('fileManage.dedupType') },
  { value: 'unique', label: t('fileManage.uniqueType') }
])
...
const handleDedupFilterChange = async (event: Event) => {
  const target = event.target as HTMLSelectElement
  await setDedupFilter(target.value as AdminFileDedupFilter)
}
