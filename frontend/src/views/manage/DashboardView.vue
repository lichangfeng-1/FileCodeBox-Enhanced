  ...
  <div class="p-6">
    <div class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      ...
      <button
        ...
        class="theme-surface inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-70"
      >
    ...
    <div
      v-if="errorMessage"
      class="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200"
    >
      {{ errorMessage }}
    </div>
    ...
    <div v-if="dashboardData.hasExtendedStats" class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
      <section class="xl:col-span-2 rounded-lg p-5 shadow-sm" :class="[panelClass]">
        <div class="mb-5 flex items-center justify-between">
          ...
          <ActivityIcon class="theme-text-muted h-5 w-5" />
        </div>
    ...
    <footer
      class="theme-divider theme-text-muted mt-6 flex flex-col gap-2 border-t pt-4 text-xs sm:flex-row sm:items-center sm:justify-between"
    >
      <span>{{ t('admin.dashboard.footerProduct') }}</span>
      <span class="flex flex-wrap items-center justify-end gap-2">
        <span>{{ t('admin.dashboard.backendVersion') }}</span>
        <span
          class="theme-surface rounded-md border px-2 py-0.5 font-medium"
        >
          {{ versionText }}
        </span>
...
const primaryTextClass = 'theme-text-strong'
const mutedTextClass = 'theme-text-muted'
const versionText = computed(() => appVersion.value || t('admin.dashboard.versionPending'))
const panelClass = 'theme-surface rounded-xl border'
const subtlePanelClass = 'theme-divider border rounded-lg'
...
const getHealthActionClass = (tone: DashboardHealthAction['tone']) => {
  const toneMap: Record<DashboardHealthAction['tone'], string> = {
    danger: 'border-red-100 bg-red-50 text-red-700 hover:border-red-200 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-300',
    warning: 'border-amber-100 bg-amber-50 text-amber-700 hover:border-amber-200 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-300',
    success: 'border-slate-200 bg-slate-50 text-slate-700 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-300',
    neutral: 'border-gray-100 bg-gray-50 text-gray-700 hover:border-gray-200 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-300'
  }
  return toneMap[tone]
}
