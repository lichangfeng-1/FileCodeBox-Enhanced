...
const adminFileDedupFilters: AdminFileDedupFilter[] = ['all', 'dedup', 'unique']
...
export function useAdminFiles() {
  ...
  const requestParams = computed<AdminFileListParams>(() => ({
    ...
    dedup: params.value.dedup === 'all' ? undefined : params.value.dedup,
  ...
  const createFileViewItem = (file: FileListItem): AdminFileViewItem => {
    ...
    return {
      ...
      isExpiredFile,
      isChunkedFile,
      isDedupFile: Boolean((file as unknown as Record<string, unknown>).isDedup),
  ...
  const createFileDetailViewItem = (
    ...
    return {
      ...
      auditLogs: (detail as unknown as Record<string, unknown>).auditLogs as AdminFileAuditLogItem[] || (detail as unknown as Record<string, unknown>).audit_logs as AdminFileAuditLogItem[] || [],
      downloadCount: Number((detail as unknown as Record<string, unknown>).downloadCount ?? (detail as unknown as Record<string, unknown>).download_count ?? 0)
    }
  }
  ...
  const setDedupFilter = async (dedup: AdminFileDedupFilter) => {
    markViewPresetDirty()
    params.value.dedup = dedup
    params.value.page = 1
    await loadFiles()
  }
