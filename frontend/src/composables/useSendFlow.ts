...
export function useSendFlow() {
  ...
  const sendType = computed<SendType>(() => {
    // 智能判断：有文件则为文件模式（文本作为备注），否则为纯文本模式
    return (selectedFiles.value.length > 0 || !!selectedFile.value) ? 'file' : 'text'
  })
    ...
    try {
      ...
      if (sendType.value === 'file') {
        response = await submitFile({
          ...
          text: textContent.value.trim() || undefined
        })
      } else {
