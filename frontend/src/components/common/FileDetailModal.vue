    ...
    >
      ...
      >
        ...
        <div class="space-y-4" v-if="record">
          ...
          <!-- 文件备注（发件时填写的文本） -->
          <div v-if="record.content && record.filename !== 'Text'" class="flex items-start">
            <FileTextIcon
              class="mr-3 mt-0.5 h-5 w-5 flex-shrink-0 sm:h-6 sm:w-6"
              :class="isDarkMode ? 'text-zinc-200' : 'text-zinc-800'"
            />
            <div class="min-w-0 flex-grow" :class="isDarkMode ? 'text-zinc-300' : 'text-slate-700'">
              <span class="font-medium">备注：</span>
              <div
                class="mt-1 whitespace-pre-wrap text-sm opacity-80 overflow-y-auto rounded-lg border px-3 py-2 transition-all"
                :class="[
                  noteExpanded ? 'max-h-80' : 'max-h-24',
                  isDarkMode ? 'border-zinc-700 bg-zinc-800/50' : 'border-slate-200 bg-slate-50/80'
                ]"
              >
                {{ record.content }}
              </div>
              <button
                v-if="record.content.length > 100"
                class="mt-1 text-xs underline opacity-60 hover:opacity-100 transition-opacity"
                @click="noteExpanded = !noteExpanded"
              >
                {{ noteExpanded ? '收起' : '展开全文' }}
              </button>
            </div>
          </div>
          <div class="flex items-center">
...
import { FileIcon, CalendarIcon, HardDriveIcon, DownloadIcon, FileTextIcon } from 'lucide-vue-next'
