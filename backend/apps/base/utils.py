...
async def get_expire_info(
    ...
    expire_styles = {
        ...
        "count": lambda: (now + datetime.timedelta(days=int(getattr(settings, "count_expire_days", 7))), expire_value),
    ...
    if expire_style in expire_styles:
        ...
        if isinstance(result, tuple):
            ...
            expired_at = result
        if expired_at and max_timedelta and expired_at - now > max_timedelta:
...
ip_limit = {
    ...
    # SEC-005: 管理员登录防暴力破解（15分钟内失败5次→锁定）
    "login": IPRateLimit(count=5, minutes=15),
}
