...
class FileUploadService:
    ...
    async def create_file_record(
        ...
        **extra_fields,
        ...
        """统一创建FileCodes记录，返回创建的对象（含 id 和 code）"""
...
# SEC-006: 危险文件类型的魔数签名（文件头指纹）
# 采用黑名单策略（只拦危险类型），避免误杀正常文件
DANGEROUS_MAGIC = [
    (b"MZ", "Windows可执行文件"),           # .exe / .dll
    (b"\x7fELF", "Linux可执行文件"),        # ELF
    (b"#!", "脚本文件"),                    # shell 脚本
    (b"\xd0\xcf\x11\xe0", "Office宏文档"),  # 旧版 Office（可含宏病毒）
]

# SEC-006 增强：常见文件类型的魔数→扩展名映射（用于检测扩展名与内容是否匹配）
KNOWN_TYPE_MAGIC = [
...
def _detect_type_mismatch(head: bytes, filename: str) -> Optional[str]:
    """检测文件扩展名与实际内容是否匹配。

    返回 None 表示匹配/无法判断，返回字符串表示不匹配的描述。
    """
    import os
    ext = os.path.splitext(filename)[1].lower()  # 如 ".jpg"
    if not ext:
        return None
    for magic, valid_exts in KNOWN_TYPE_MAGIC:
        if head.startswith(magic):
            if ext not in valid_exts:
                expected = "/".join(sorted(valid_exts))
                return f"文件内容为 {expected} 格式，但扩展名为 {ext}"
            return None  # 匹配
    return None  # 未识别的类型，不做判断


def _is_real_type_allowed(head: bytes, filename: str) -> bool:
    """检测文件真实类型是否在管理员白名单内。

    用于防止“改扩展名绕过白名单”：即使扩展名骗过了第一层检查，
    真实类型不在白名单内仍会被拦截。
    白名单为 ["*"] 时允许所有类型。
    """
    allowed = normalize_allowed_file_types()
    if "*" in allowed or "*/*" in allowed:
        return True  # 管理员允许所有类型
    ...
    for magic, valid_exts in KNOWN_TYPE_MAGIC:
        if head.startswith(magic):
            ...
            for ext in valid_exts:
                bare = ext.lstrip(".")  # ".jpg" → "jpg"
                if ext in allowed or bare in allowed:
                    return True
                # 也检查 MIME 通配（如 "image/*"）
                for rule in allowed:
                    if "/" in rule and fnmatchcase(ext, rule):
                        return True
            return False  # 真实类型不在白名单
    return True  # 未识别类型，不做判断（避免误杀）


async def validate_file_content(file: UploadFile) -> Optional[str]:
    """SEC-006: 校验文件真实类型（读取前 8 字节判断 magic bytes）。

    防止攻击者将可执行文件改名绕过扩展名检查。
    读取后自动 seek 回文件开头，不影响后续保存。

    返回：警告信息（扩展名不匹配）或 None（无警告）。
     Raises：HTTPException 400（危险文件）或 403（严格模式+不匹配）。
    """
    head = await file.read(8)
    await file.seek(0)  # 重置文件指针，保证后续完整读取

    # 第一层：危险文件检测（受管理员设置控制）
    dangerous_warning = None
    for magic, desc in DANGEROUS_MAGIC:
        if head.startswith(magic):
            if int(getattr(settings, "blockDangerousTypes", 0)):
                raise HTTPException(status_code=400, detail=f"禁止上传{desc}")
            dangerous_warning = f"检测到{desc}内容"
            break

    # 第二层：用真实类型核对管理员白名单（防改名绕过）
    mismatch = _detect_type_mismatch(head, file.filename or "")
    if mismatch:
        # 检测到了真实类型与扩展名不符——检查真实类型是否在管理员允许的范围内
        real_allowed = _is_real_type_allowed(head, file.filename or "")
        if not real_allowed:
            # 真实类型不在白名单 → 无论扩展名怎么写，拦截
            raise HTTPException(status_code=403, detail=f"文件类型不匹配：{mismatch}")
        # 真实类型在白名单内（如 .jpeg 改名 .jpg）→ 仅提示
        strict = int(getattr(settings, "fileTypeStrict", 0))
        if strict:
            raise HTTPException(status_code=403, detail=f"文件类型不匹配：{mismatch}")
        return mismatch  # 非严格模式：返回警告，不拦截
    return dangerous_warning  # 无扩展名不匹配，但可能有危险文件提示
...
async def share_text(
    ...
    # SEC-002: 与 share_file 保持一致，校验过期方式是否在管理员允许的白名单内
    ...
    await record_audit(
        event_type="upload", ip=ip, request=request,
        file_code=code, file_name="文本分享",
        detail={"type": "text", "size": text_size},
    )
    await emit_webhook("file.uploaded", {
        "code": code, "name": "文本分享", "size": text_size, "type": "text",
    })
...
async def share_file(
    ...
    type_warning = await validate_file_content(file)  # SEC-006: magic bytes 深度校验
    if expire_style not in settings.expireStyle:
        raise HTTPException(status_code=400, detail="过期时间类型错误")
    ...
    try:
        ...
        file_code_obj = await create_file_code(
            code=code,
            prefix=prefix,
            suffix=suffix,
    ...
    await record_audit(
        event_type="upload", ip=ip, request=request,
        file_id=file_code_obj.id, file_code=code, file_name=file.filename,
        detail={"type": "file", "size": file_size},
    )
    await emit_webhook("file.uploaded", {
        "code": code, "name": file.filename, "size": file_size, "type": "file",
    })
    return APIResponse(detail={"code": code, "name": file.filename, "warning": type_warning})
...
async def download_file(key: str, code: str, request: Request = None, ip: str = Depends(ip_limit["error"])):
    file_storage: FileStorageInterface = storages[settings.file_storage]()
    normalized_code = normalize_share_code(code)
    # BUG-002: 同时校验当前窗口与上一个窗口的 token，修复时间窗口边界竞态
    # （用户在某窗口末尾获取的 token，请求到达时可能已进入下一窗口）
    ...
    if not await consume_file_usage(file_code):
        return APIResponse(code=404, detail="文件已过期")
    await record_audit(
        event_type="download", ip=ip, request=request,
        file_id=file_code.id, file_code=file_code.code,
        file_name=f"{file_code.prefix}{file_code.suffix}",
    )
    await emit_webhook("file.retrieved", {
        "code": file_code.code,
        "name": f"{file_code.prefix}{file_code.suffix}",
        "size": file_code.size,
        "retriever_ip": ip,
        "remaining_count": file_code.expired_count,
    })
    return (
...
@share_api.post("/dedup/check", dependencies=[Depends(share_required_login)])
async def dedup_check(
    request: Request,
    data: DedupCheckModel, ip: str = Depends(ip_limit["upload"])
):
    """秒传检查：通过文件哈希查询是否已存在相同文件，命中时创建新分享记录"""
    import re

    if not settings.enableDedup:
        return APIResponse(detail={"existed": False})
    ...
    # 安全校验：hash 必须为合法 SHA256（64位十六进制）
    if not re.fullmatch(r"[0-9a-fA-F]{64}", data.file_hash):
        return APIResponse(detail={"existed": False})

    if data.expire_style not in settings.expireStyle:
        raise HTTPException(status_code=400, detail="过期时间类型错误")

    # 双重校验：hash + size 必须同时匹配（不管是否有备注）
    existing = await FileCodes.filter(
        file_hash=data.file_hash,
        size=data.file_size,
    ).first()

    if existing and not await existing.is_expired():
        file_storage: FileStorageInterface = storages[settings.file_storage]()
        if await file_storage.file_exists(await existing.get_file_path()):
            # 创建新的分享记录（新取件码、新过期策略），复用物理文件
            expired_at, expired_count, used_count, code = await get_expire_info(
                data.expire_value, data.expire_style
            )
            file_code_obj = await FileCodes.create(
                code=code,
                prefix=existing.prefix,
                suffix=existing.suffix,
                uuid_file_name=existing.uuid_file_name,
                ...
                expired_count=expired_count,
                used_count=used_count,
                text=data.text.strip() or None,
            )
            # REL-003: 二次验证文件仍存在（缩小 TOCTOU 竞态窗口）
            # 若文件在创建记录瞬间被删除（清理任务/管理员操作），回滚记录
            if not await file_storage.file_exists(await file_code_obj.get_file_path()):
                await file_code_obj.delete()
                return APIResponse(detail={"existed": False})
            ip_limit["upload"].add_ip(ip)
            file_name = f"{existing.prefix}{existing.suffix}"
            await record_audit(
                event_type="upload", ip=ip, request=request,
                file_id=file_code_obj.id, file_code=code, file_name=file_name,
                detail={
                    "type": "file", "size": existing.size,
                    "mode": "dedup", "note": f"秒传（源文件: {existing.code}）",
                    "source_code": existing.code, "source_file_id": existing.id,
                },
            )
            await emit_webhook("file.uploaded", {
                "code": code, "name": file_name, "size": existing.size,
                "type": "file", "mode": "dedup", "source_code": existing.code,
            })
            return APIResponse(
                detail={
                    "existed": True,
...
async def upload_chunk(
    ...
    # SEC-006: 第一个分片包含文件头，校验 magic bytes
    if chunk_index == 0:
        await validate_file_content(chunk)
...
async def complete_upload(
    ...
    if not chunk_info:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="上传会话不存在")
    # SEC-003: 与 share_file 保持一致，校验过期方式是否在管理员允许的白名单内
    if data.expire_style not in settings.expireStyle:
        raise HTTPException(status_code=400, detail="过期时间类型错误")
    await reserve_storage(
        f"chunk:{upload_id}",
    ...
    try:
        ...
        ip_limit["upload"].add_ip(ip)
        await record_audit(
            event_type="upload", ip=ip, request=None,
            file_id=file_code_obj.id, file_code=code, file_name=chunk_info.file_name,
            detail={"type": "file", "size": chunk_info.file_size, "mode": "chunk"},
        )
...
async def presign_upload_proxy(
    upload_id: str, request: Request, file: UploadFile = File(...), ip: str = Depends(ip_limit["upload"])
):
    """代理模式上传，服务器转存到存储后端"""
    ...
    try:
        file_code_obj = await FileUploadService.create_file_record(
            session.file_name,
            file_size,
    ...
    await record_audit(
        event_type="upload", ip=ip, request=request,
        file_id=file_code_obj.id, file_code=file_code_obj.code, file_name=session.file_name,
        detail={"type": "file", "size": file_size, "mode": "presign_proxy"},
    )
    return APIResponse(detail={"code": file_code_obj.code, "name": session.file_name})


@presign_api.post(
    "/upload/confirm/{upload_id}", dependencies=[Depends(share_required_login)]
)
async def presign_upload_confirm(upload_id: str, request: Request, ip: str = Depends(ip_limit["upload"])):
    ...
    await record_audit(
        event_type="upload", ip=ip, request=request,
        file_id=file_code_obj.id, file_code=file_code_obj.code, file_name=session.file_name,
        detail={"type": "file", "size": session.file_size, "mode": "presign_direct"},
    )
    return APIResponse(detail={"code": file_code_obj.code, "name": session.file_name})
