...
class UploadChunk(models.Model):
    ...
    text = fields.TextField(null=True)  # 文件备注（仅 chunk_index=-1 的会话记录使用）
    created_at = fields.DatetimeField(auto_now_add=True)
...
class PresignUploadSession(models.Model):
    ...
    text = fields.TextField(null=True)  # 文件备注
    created_at = fields.DatetimeField(auto_now_add=True)
...
class AuditLog(models.Model):
    """审计日志：记录上传/下载/取件/管理等操作"""

    id = fields.IntField(pk=True)
    event_type = fields.CharField(max_length=32, index=True)
    # 事件类型: upload, download, retrieve, expire, delete, admin_login

    file_id = fields.IntField(null=True, index=True)
    file_code = fields.CharField(max_length=255, null=True)
    file_name = fields.CharField(max_length=512, null=True)

    # 客户端信息
    ip = fields.CharField(max_length=45, index=True)
    ip_location = fields.CharField(max_length=255, null=True)
    user_agent = fields.TextField(null=True)
    browser = fields.CharField(max_length=64, null=True)
    browser_version = fields.CharField(max_length=32, null=True)
    os = fields.CharField(max_length=64, null=True)
    os_version = fields.CharField(max_length=32, null=True)
    device_type = fields.CharField(max_length=16, null=True)
...
class WebhookConfig(models.Model):
    ...
    events = fields.JSONField()  # ["file.retrieved", "file.expired", "file.uploaded"]
    headers = fields.JSONField(null=True)  # {"Authorization": "Bearer xxx"}
...
class WebhookLog(models.Model):
    ...
    id = fields.IntField(pk=True)
    webhook_id = fields.IntField(index=True)
