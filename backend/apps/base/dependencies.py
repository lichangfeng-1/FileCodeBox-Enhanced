...
class IPRateLimit:
    # REL-002: 内存上限，防止僵尸网络用海量不同 IP 撑爆内存
    ...
    def add_ip(self, ip: str) -> int:
        ...
        self.ips[ip] = ip_info
        # REL-002: 超过上限时清理最旧的 25% 条目
        if len(self.ips) > self.MAX_ENTRIES:
            self._evict_oldest()
        return ip_info["count"]

    def _evict_oldest(self) -> None:
        """清理最旧的 25% 条目，避免内存无限增长"""
        evict_count = self.MAX_ENTRIES // 4
        oldest_ips = sorted(self.ips, key=lambda k: self.ips[k]["time"])[:evict_count]
