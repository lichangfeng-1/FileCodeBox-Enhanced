...
# QUA-001: JWT 标准 base64url 编码（RFC 7519）
...
def verify_token(token: str) -> dict:
    """
    验证JWT token（兼容旧版标准 base64 和新版 base64url）
    ...
    try:
        ...
        # 新版 base64url 签名
        expected_b64url = _b64url_encode(expected_signature)
        # 旧版标准 base64 签名（兼容期）
        expected_std = base64.b64encode(expected_signature).decode()

        if not (
            hmac.compare_digest(signature_b64, expected_b64url)
            or hmac.compare_digest(signature_b64, expected_std)
        ):
            raise ValueError("无效的签名")

        # 解码 payload（先尝试 base64url，失败则回退标准 base64）
        try:
...
async def admin_required(
    ...
    验证管理员权限
    支持两种方式：
    1. Authorization: Bearer <token> 请求头（正常 API 调用）
    2. ?token=<token> URL 参数（CSV 导出等无法设置 Header 的场景）
    """
