...
def build_public_config() -> dict:
    return {
        ...
        "icp_number": getattr(settings, "icp_number", "") or "",
    }
...
async def load_config():
    ...
    await refresh_settings(force=True)  # 启动时强制加载
    await ensure_security_settings()

    # 自动初始化：如果环境变量提供了 ADMIN_PASSWORD 且系统未初始化，自动完成设置
    await auto_initialize_from_env()
...
async def auto_initialize_from_env():
    """从环境变量自动初始化系统（无需手动访问初始化页面）
    注意：此函数已在 db_startup_lock() 内被调用，不能再调用 initialize_system()（会重复加锁）
    """
    import os
    from core.security import generate_jwt_secret, is_valid_jwt_secret
    ...
    if len(admin_password) < 8:
        logger.warning("环境变量 ADMIN_PASSWORD 不足 8 位，跳过自动初始化")
        return
    try:
        # 直接执行初始化逻辑（不再调用 initialize_system 避免嵌套锁）
        ...
        next_config["admin_token"] = hash_password(admin_password)
        ...
        await refresh_settings(force=True)  # 初始化后强制加载
        logger.info("系统已通过环境变量自动初始化")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    # SEC-004: 前后端同源部署（Nginx 反代）且使用 Bearer Token 认证（非 Cookie），
    # 无需跨域携带凭证。allow_origins=["*"] 与 allow_credentials=True 同开是危险且违规的组合，故关闭 credentials。
