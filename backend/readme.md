...
> 基于 [FileCodeBox v2.5.0](https://github.com/vastsa/FileCodeBox) 二次开发，新增以下生产级能力：

| 功能 | 说明 | 配置项 |
|------|------|--------|
| 🗄️ **多数据库支持** | PostgreSQL（默认）/ MySQL / SQLite，环境变量配置 | `DATABASE_URL` |
| 📝 **全面审计系统** | 记录上传/下载/登录的 IP、设备、浏览器、地理位置 | `enableAudit` |
| 🔔 **Webhook 事件通知** | 文件上传/取走/过期时回调外部系统，3次重试+SSRF防护 | `enableWebhook` |
| ⚡ **秒传/文件去重** | SHA256+文件大小双重校验，命中时瞬间生成取件码 | `enableDedup` |
| 🌐 **信任代理配置** | 管理后台配置反向代理 IP，审计获取真实客户端 IP | `trustedProxies` |
| 🚫 **禁止爬虫索引** | 一键开启 noindex/nofollow，屏蔽搜索引擎 | `noindex` |
| 📊 **审计 CSV 导出** | 管理后台一键导出审计日志，支持多维筛选 | - |
| 🛡️ **安全加固** | 熍断器、UA截断、/0全网段拦截、hash格式校验 | - |
| 🇨🇳 **国内环境适配** | Docker 构建自动检测网络，切换 npm/pip/GitHub 镜像源 | 自动 |
| 🖥️ **前端管理界面** | 审计日志、Webhook 管理已集成到管理后台侧边栏 | - |

### 部署方式

```bash
# 使用 Docker Compose（内置 PostgreSQL）
docker compose up --build -d

# 接入已有数据库
DATABASE_URL=postgres://user:pass@db-host:5432/filecodebox docker compose up --build -d

# 回退到 SQLite（零配置）
DB_TYPE=sqlite docker compose up --build -d
```

### 反向代理支持

支持 Nginx / Lucky666 等反向代理，配置信任代理后审计系统自动获取真实客户端 IP。
详见：[Lucky666 反向代理配置指南](../docs/REVERSE_PROXY_LUCKY.md)

### 前端二开

前端代码已合并至本仓库 `frontend/` 目录，Docker 构建时从本地编译，无需访问 GitHub。
管理后台新增两个页面：
- **审计日志**（`/#/admin/audit`）：多维筛选、分页、CSV 导出
- **Webhook 通知**（`/#/admin/webhook`）：配置 CRUD、测试发送、发送日志

---

## 🖼️ 界面预览

> 前端源码已集成至本仓库 `frontend/` 目录（基于 [2024主题](https://github.com/vastsa/FileCodeBoxFronted) 二次开发）
