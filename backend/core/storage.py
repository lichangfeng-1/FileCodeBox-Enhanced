    ...
    async def get_file_response(self, file_code: FileCodes):
        try:
            ...
            return StreamingResponse(
                ...
                media_type="application/octet-stream",
                headers=headers,
                background=BackgroundTask(session.close),  # BUG-003: 客户端中断时兜底关闭会话
            )
...
class OneDriveFileStorage(FileStorageInterface):
    ...
    def _get_path_str(self, path):
        ...
        # QUA-002: 仅移除最后一个扩展名（保留 .tar.gz 等多重扩展名，不处理隐藏文件）
        name = path[-1]
        dot_index = name.rfind(".")
        if dot_index > 0:  # dot_index==0 表示隐藏文件（.bashrc），不截断
            path[-1] = name[:dot_index]
        return "/".join(path)
